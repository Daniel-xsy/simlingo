"""
Interactive agent based on agent_simlingo.py
Supports keyboard interruption to manually select commands (1-6) at any timestamp.
If no input is provided, uses the previous command.
"""
...
# Import RoadOption for command mapping
try:
    from agents.navigation.local_planner import RoadOption
except ImportError:
    # Fallback if RoadOption is not available
...
class LingoAgentInteractive(autonomous_agent.AutonomousAgent):
    """
    Interactive version of LingoAgent that supports keyboard interruption
    to manually select commands at any timestamp.
    """
    ...
    def setup(self, path_to_conf_file, route_index=None):
        ...
        # Force eval_route_as to 'command' for interactive mode
        ...
        # Interactive mode: user-selected command
        self.user_selected_command = None  # RoadOption enum value
        self.last_used_command = None  # Keep track of last used command
        ...
        # RoadOption mapping (1-6 to RoadOption enum)
        self.command_to_roadoption = {
            1: RoadOption.LEFT,
            2: RoadOption.RIGHT,
        ...
        print("="*60)
        print("Press a number key (1-6) at any time to select a command:")
        print("  1: go left at the next intersection")
        print("  2: go right at the next intersection")
        print("  3: go straight at the next intersection")
        print("  4: follow the road")
        print("  5: do a lane change to the left")
        print("  6: do a lane change to the right")
        print("Press Enter without a number to use the previous command")
        print("="*60 + "\n")
    ...
    def _check_keyboard_input(self):
        ...
        Returns:
            int 1-6 for preset commands,
            'i' to enter a custom instruction,
            None if no/invalid input.
        """
            ...
            try:
                ...
                try:
                    ...
                    if 1 <= cmd_num <= 6:
                        ...
                        print("Invalid command: enter 1-6 for preset or 'i' for custom text.", flush=True)
                        return None
                except ValueError:
                    print("Invalid input: enter 1-6 for preset or 'i' for custom text.", flush=True)
                    return None
    ...
    def tick(self, input_data):
        ...
        # Get waypoint route from planner
        waypoint_route = self._route_planner.run_step(np.append(result['gps'], gps_pos[2]))

        # Extract waypoints from route planner (we'll override the command later)
        ...
        user_cmd_num = self._check_keyboard_input()
        if user_cmd_num == "i":
            # Blocking prompt for free-form instruction
            try:
                custom_text = input("\n[INTERACTIVE] Type custom driving instruction: ").strip()
            except (EOFError, KeyboardInterrupt):
                custom_text = ""

            if custom_text:
                self.custom_prompt = custom_text
                # keep the maneuver token consistent; default to last command or lanefollow
                fallback_cmd = self.last_used_command or RoadOption.LANEFOLLOW
                self.user_selected_command = fallback_cmd
                self.last_used_command = self.user_selected_command
                print(f"[INTERACTIVE] Using custom instruction: '{custom_text}'", flush=True)
            else:
                print("[INTERACTIVE] Empty instruction ignored; keeping previous command.", flush=True)
                if self.last_used_command is None:
                    # fall back to route planner command
                    if len(waypoint_route) > 2:
                        _, far_command = waypoint_route[1]
                    elif len(waypoint_route) > 1:
                        _, far_command = waypoint_route[1]
                    else:
                        _, far_command = waypoint_route[0]
                    self.last_used_command = far_command
                self.user_selected_command = self.last_used_command

        elif user_cmd_num is not None:
            # User provided a new preset command
            self.user_selected_command = self.command_to_roadoption[user_cmd_num]
            self.last_used_command = self.user_selected_command
            # Clear any lingering custom prompt
            self.custom_prompt = None
            print(f"\n[INTERACTIVE] User selected command {user_cmd_num}: {self.map_command[user_cmd_num]}", flush=True)
        elif self.last_used_command is not None:
            # Use the last user-selected command
            self.user_selected_command = self.last_used_command
        else:
            # No user input yet, use route planner's command
            if len(waypoint_route) > 2:
                _, far_command = waypoint_route[1]
                _, next_far_command = waypoint_route[2]
            elif len(waypoint_route) > 1:
                _, far_command = waypoint_route[1]
                _, next_far_command = waypoint_route[1]
            else:
                _, far_command = waypoint_route[0]
                _, next_far_command = waypoint_route[0]
            
            # Initialize last_used_command with route planner's command if not set
            if self.last_used_command is None:
                self.last_used_command = far_command
            self.user_selected_command = self.last_used_command

        # Override the route planner's command with user-selected command
        # Keep the waypoints from route planner, but use user's command
        far_command = self.user_selected_command
        next_far_command = self.user_selected_command  # Use same command for next
        ...
        # Always use 'command' mode in interactive mode
        ...
        if self.LMDRIVE_AUGM:
            lmdrive_index = random.choice(command_template_mappings[far_command.value])
            ...
            command = self.map_command[far_command.value]
            next_command = self.map_command[next_far_command.value]
            if self.last_command in [1, 2, 3] and far_command.value == 4:
                next_command = command
                command = self.map_command[self.last_command]
            ...
            if self.custom_prompt:
                prompt_tp = f'Command: {self.custom_prompt}'
            else:
